...
# ======================================12-8 model2=== set the deconv layer at the end of network=====
...
class FSRCNN(object):
    ...
    def build_model(self):
        ...
        self.weights = {
            ...
            expand_weight: tf.Variable(tf.random_normal([1, 1, d, s], stddev=0.189, dtype=tf.float32),
                                       name=expand_weight),
            deconv_weight: tf.Variable(tf.random_normal([9, 9, 1, s], stddev=0.0001, dtype=tf.float32),
                                       name=deconv_weight)
        }
        ...
        for i in range(3, m + 3):
            ...
            self.weights[weight_name] = tf.Variable(tf.random_normal([3, 3, d, d], stddev=0.1179, dtype=tf.float32),
                                                    name=weight_name)
    ...
    def model1(self):
        # ===============================Feature Extraction===============================================================##
        conv_feature = self.prelu(
            tf.nn.conv2d(self.images, self.weights['w1'], strides=[1, 1, 1, 1], padding='VALID') + self.biases['b1'], 1)
        print(conv_feature)
        # Shrinking
        conv_shrink = self.prelu(
            tf.nn.conv2d(conv_feature, self.weights['w2'], strides=[1, 1, 1, 1], padding='SAME') + self.biases['b2'], 2)
        ...
        conv_expand = self.prelu(tf.nn.conv2d(prev_layer, conv_s_w2, strides=[1, 1, 1, 1], padding='SAME') + conv_s_b2,
                                 7)
        ...
        conv_deconv = tf.nn.conv2d_transpose(conv_expand, dconv_s_w2, output_shape=deconv_output, strides=deconv_stride,
                                             padding='SAME') + dconv_s_b2
    ...
    def model(self):
        ...
        conv_s_w1 = tf.get_variable("conv_s_w1", [5, 5, 1, 4],
        ...
        conv_feature = self.prelu(
            tf.nn.conv2d(self.images, conv_s_w1, strides=[1, 1, 1, 1], padding='VALID') + conv_s_b1, 1)

        conv_s_w2 = tf.get_variable("conv_s_w2", [3, 3, 4, 32],
                                    initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9)))
        conv_s_b2 = tf.get_variable("conv_s_b2", [32], initializer=tf.constant_initializer(0))
        conv_feature = self.prelu(
            tf.nn.conv2d(conv_feature, conv_s_w2, strides=[1, 1, 1, 1], padding='SAME') + conv_s_b2, 2)

        conv_s_w3 = tf.get_variable("conv_s_w3", [5, 5, 1, 32],
                                    initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        ##=======================================================================================================================
        #############################     Deep Network  ########################################################################
        conv_D_Fw1 = tf.get_variable("conv_D_Fw1", [5, 5, 1, 64],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9)))
        conv_D_Fb1 = tf.get_variable("conv_D_Fb1", [64], initializer=tf.constant_initializer(0))
        tensorD = tf.nn.bias_add(tf.nn.conv2d(self.images, conv_D_Fw1, strides=[1, 1, 1, 1], padding='VALID'),
                                 conv_D_Fb1)
        tensorD = tf.nn.relu(tensorD)

        conv_D_Fw11 = tf.get_variable("conv_D_Fw11", [5, 5, 64, 64],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9)))
        ...
        conv_D_Mw1 = tf.get_variable("conv_D_Mw1", [1, 1, 416, 12],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_Mw2 = tf.get_variable("conv_D_Mw2", [3, 3, 12, 12],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_Mw3 = tf.get_variable("conv_D_Mw3", [3, 3, 12, 12],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_Mw4 = tf.get_variable("conv_D_Mw4", [3, 3, 88, 12],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_Mw5 = tf.get_variable("conv_D_Mw5", [3, 3, 12, 12],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_MUw1 = tf.get_variable("conv_D_MUw1", [3, 3, 12, 64],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        conv_D_MUb1 = tf.get_variable("conv_D_MUb1", [64], initializer=tf.constant_initializer(0))
        tensorD_1 = tf.nn.bias_add(tf.nn.conv2d(tensorD, conv_D_MUw1, strides=[1, 1, 1, 1], padding='SAME'),
                                   conv_D_MUb1)
        tensorD_1 = tf.nn.relu(tensorD_1)

        conv_D_MUw2 = tf.get_variable("conv_D_MUw2", [3, 3, 64, 64],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        conv_D_MUb2 = tf.get_variable("conv_D_MUb2", [64], initializer=tf.constant_initializer(0))
        tensorD = tf.nn.bias_add(tf.nn.conv2d(tensorD_1, conv_D_MUw2, strides=[1, 1, 1, 1], padding='SAME'),
                                 conv_D_MUb2)
        tensorD = tf.nn.relu(tensorD)
        ...
        conv_D_MUw3 = tf.get_variable("conv_D_MUw3", [3, 3, 128, 64],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        conv_D_MUb3 = tf.get_variable("conv_D_MUb3", [64], initializer=tf.constant_initializer(0))
        tensorD_3 = tf.nn.bias_add(tf.nn.conv2d(tensorD, conv_D_MUw3, strides=[1, 1, 1, 1], padding='SAME'),
                                   conv_D_MUb3)
        tensorD_3 = tf.nn.relu(tensorD_3)

        conv_D_MUw4 = tf.get_variable("conv_D_MUw4", [3, 3, 64, 64],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        conv_D_MUb4 = tf.get_variable("conv_D_MUb4", [64], initializer=tf.constant_initializer(0))
        tensorD = tf.nn.bias_add(tf.nn.conv2d(tensorD_3, conv_D_MUw4, strides=[1, 1, 1, 1], padding='SAME'),
                                 conv_D_MUb4)
        tensorD = tf.nn.relu(tensorD)
        ...
        conv_D_MUw5 = tf.get_variable("conv_D_MUw5", [1, 1, 128, 16],
                                      initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_MUw5_1 = tf.get_variable("conv_D_MUw5_1", [1, 1, 16, 4],
                                        initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_MUw5_2 = tf.get_variable("conv_D_MUw5_2", [3, 3, 16, 4],
                                        initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_MUw5_3 = tf.get_variable("conv_D_MUw5_3", [5, 5, 16, 4],
                                        initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_D_MUw5_4 = tf.get_variable("conv_D_MUw5_4", [7, 7, 16, 4],
                                        initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        ##===================== UPsampling =================================================================================
        conv_D_Uw1 = tf.get_variable("conv_D_Uw1", [1, 1, 1, 64],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        conv_Lastw = tf.get_variable("conv_Lastw", [1, 1, 1, 1],
                                     initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
    ...
    def prelu(self, _x, i):
        ...
        """
        alphas = tf.get_variable('alpha{}'.format(i), _x.get_shape()[-1], initializer=tf.constant_initializer(0.0),
                                 dtype=tf.float32)
        pos = tf.nn.relu(_x)
