...
def train_input_setup(config):
  ...
  for i in xrange(len(data)):
   ...
   # for x in range(0, h - image_size - padding + 1, stride):       #for scale=3 and 4  ,for scale 2 error
    for x in range(0, h - image_size - padding + 1 - 18, stride):   #for scale=2
      for y in range(0, w - image_size - padding + 1 - 18, stride):  #for scale=2
      #for y in range(0, w - image_size - padding + 1, stride):      #for scale=3 and 4  ,for scale 2 error
        sub_input = input_[x + padding : x + padding + image_size, y + padding : y + padding + image_size]
        x_loc, y_loc = x + label_padding, y + label_padding
       # print(x_loc)
       # print(y_loc)
...
def test_input_setup(config):
  ...
  nx, ny = 0, 0
 # for x in range(0, h - image_size - padding + 1 - 18, stride):    # right for scale 2 3 4
  for x in range(0, h - image_size - padding + 1 , stride):      #origin   error
    nx += 1
    ny = 0
    #for y in range(0, w - image_size - padding + 1 - 18, stride):
    for y in range(0, w - image_size - padding + 1 , stride):    #origin
      ny += 1
      sub_input = input_[x + padding : x + padding + image_size, y + padding : y + padding + image_size]
      x_loc, y_loc = x + label_padding, y + label_padding
      sub_label = label_[x_loc * scale : x_loc * scale + label_size, y_loc * scale : y_loc * scale + label_size]
