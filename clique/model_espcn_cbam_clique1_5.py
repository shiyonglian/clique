...
class RDN(object):
    ...
    def model(self):
        ...
        # tensorD = tf.nn.leaky_relu(tensorD,alpha=0.2,name=None)
        ...
        tensorD = tf.nn.relu(tensorD)
        tensorD1 = cbam_block(tensorD, 'tensorD1', 4)
        
        # tensorD = tf.nn.leaky_relu(tensorD, alpha=0.2, name=None)
        # =============  clique block ==================================== # =============  dense block =================

        tensorB = build_model1(tensorD1, 64, 5, 1, 1, if_a=False, if_b=True, if_c=True)# T=12 represent conv number
        tensorD = tf.concat((tensorD1, tensorB), axis=3)
        tensorcbam = cbam_block(tensorD, 'tensorcbam', 8)

         # ==========================================MAPPING ================================================================

        conv_D_Mw1 = tf.get_variable("conv_D_Mw1", [1, 1, 160, 12], initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        conv_D_Mb1 = tf.get_variable("conv_D_Mb1", [12], initializer=tf.constant_initializer(0))
        tensorD = tf.nn.bias_add(tf.nn.conv2d(tensorcbam, conv_D_Mw1, strides=[1, 1, 1, 1], padding='SAME'), conv_D_Mb1)
        tensorD = tf.nn.relu(tensorD)
        # tensorD = tf.nn.leaky_relu(tensorD, alpha=0.2, name=None)
        ...
        # tensorD = tf.nn.leaky_relu(tensorD, alpha=0.2, name=None)
        tensorD_22 = cbam_block(tensorD_2, 'tensorD_22', 2)
        # # ======================== cbam_block ======================  #======================== cbam_block =========================

        ##===================== UPsampling =================================================================================

        tensorD_2 = tf.concat((tensorD1, tensorD_22), axis=3)
        conv_D_Uw1 = tf.get_variable("conv_D_Uw1", [1, 1, 76, 64],
        ...
        tensorD = self.UPN(tensorD)

        ##================================= Multi ==========================================================================
        conv_D_MUw5 = tf.get_variable("conv_D_MUw5", [1, 1, 3, 16],
                                              initializer=tf.random_normal_initializer(stddev=np.sqrt(2.0 / 9 / 64)))
        ...
        # tensorD5 = tf.nn.leaky_relu(tensorD5, alpha=0.2, name=None)
        ...
        # tensorD5_1 = tf.nn.leaky_relu(tensorD5_1, alpha=0.2, name=None)
        ...
        # tensorD5_2 = tf.nn.leaky_relu(tensorD5_2, alpha=0.2, name=None)
        ...
        # tensorD5_3 = tf.nn.leaky_relu(tensorD5_3, alpha=0.2, name=None)
        ...
        # tensorD5_4 = tf.nn.leaky_relu(tensorD5_4, alpha=0.2, name=None)
...
#         pdb.set_trace()
        ...
        train_loss = open('train_loss.csv', 'a+')
...
#                 summary_writer.add_summary(summary_str, counter)
    ...
    def load(self, checkpoint_dir):
        print("\nReading Checkpoints.....\n")
        model_dir = "%s_%s_%s_%s_x%s" % ("espcn_clique3_4_cbam", self.D, self.C, self.G, self.scale)
