...
class FSRCNN(object):
    ...
    def __init__(self, sess, config):
        ...
        scale_factors = [[24, 40], [17, 39], [14, 40]]
        ...
        if not self.train:
            self.stride = [10, 7, 1][self.scale - 2]
    ...
    def run_test(self):
        ...
        test_label = merge(test_label, [nx, ny])
        test_label = test_label.squeeze()
        image_path = os.path.join(os.getcwd(), self.output_dir)
        image_path = os.path.join(image_path, "test_image.png")
        array_image_save(result*255, image_path)
        psnr_score = compute_psnr(test_label, result)
        ssim_score = compute_ssim(test_label, result)
    ...
    def model(self):
        ...
        # PSNR = tf.image.psnr(self.labels, self.pred, max_val=255)
        # SSIM = tf.image.ssim_multiscale(self.labels, self.pred, max_val=255)
