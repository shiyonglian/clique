...
#根据选择的不同初始化方式进行初始化，因为这个网络中使用了不同的初始化方式
...
#tf.nn.avg_pool(value,ksize,strides,padding,data_format='NHWC',name=None) #计算池化区域中元素的平均
#value是一个四维的张量，数据维度是[batch, height, width, channels]
...
#Assert statements are a convenient way to insert debugging assertions into a program
    ...
    Wfc = tf.get_variable(name=name + '_W1', shape=[channels, channels / 2],
    ...
    Wfc = tf.get_variable(name=name + '_W2', shape=[channels / 2, channels],
                          initializer=tf.contrib.layers.xavier_initializer())
...
# 该函数只在current中被调用了，但是current没有往下传值；（该函数也包含全连接和池化层），该函数调用了attentional_transition（包含全连接和池化）
def transition(input_layer, if_a, is_train, keep_prob, name):
    ...
    # output_layer = tf.nn.avg_pool(output_layer, [1, 2, 2, 1], [1, 2, 2, 1], padding='VALID')直接把池化层去掉

    return output_layer

# compress 用1*1的卷积层对网络进行降维，该函数没有调用别的函数
def compress(input_layer, is_train, keep_prob, name):
    channels = input_layer.get_shape().as_list()[-1]
    output_layer = tf.contrib.layers.batch_norm(input_layer, scale=True, is_training=is_train, updates_collections=None)
    output_layer = tf.nn.relu(output_layer)
    filters = conv_var(kernel_size=(1, 1), in_channels=channels, out_channels=channels / 2, init_method='msra',
                       name=name)
    output_layer = tf.nn.conv2d(output_layer, filters, [1, 1, 1, 1], padding='SAME')
    output_layer = tf.nn.dropout(output_layer, keep_prob)
...
# model中调用了该函数（我使用时注释了），全局池化，该函数没有调用其他函数；在model中每个五角星后面都有一个全局池化层
...
# 该函数定义的是一个biase为 0 的卷积层，在model中给了current
def first_transit(input_layer, channels, strides, with_biase=False):
    filters = conv_var(kernel_size=(3, 3), in_channels=64, out_channels=channels, init_method='msra', name='first_tran')
...
# 该函数的功能是让五角星循环，该block内部循环1次，loop_num可设置block内部的循环次数
def loop_block(input_layer, if_b, channels_per_layer, layer_num, is_train, keep_prob, block_name, loop_num=1):
    ...
    if if_b == True:
        ...
        for layer_id in range(1, layer_num + 1):
            filters = conv_var(kernel_size=(3, 3), in_channels=channels, out_channels=channels, init_method='msra',
                               name=block_name + '-' + 'to-' + str(layer_id))
    ...
    for layer_id in range(1, layer_num + 1):
        ...
        ##  Bottle neck
        if if_b == True:
            next_layer = tf.contrib.layers.batch_norm(mid_layer, scale=True, is_training=is_train,
                                                      updates_collections=None)
...
def loop_block_I_II(input_layer, if_b, channels_per_layer, layer_num, is_train, keep_prob, block_name, loop_num=1):
    ...
    block_feature = tf.concat((input_layer, stage_II), axis=3)
    transit_feature = stage_II

    return block_feature, transit_feature

# x_value 是什么
def loop_block_X(input_layer, x_value, if_b, channels_per_layer, layer_num, is_train, keep_prob, block_name,
                 loop_num=1):
    if if_b: layer_num = layer_num / 2  ## if bottleneck is used, the T value should be multiplied by 2.
    channels = channels_per_layer
    node_0_channels = input_layer.get_shape().as_list()[-1]
